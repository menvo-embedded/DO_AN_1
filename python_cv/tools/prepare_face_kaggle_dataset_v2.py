...
PERSON_MAP = {
    ...
    "NV002_Äá»©c": {
        ...
        "employee_id": "NV002",
        "display_name": "Duc",
        "display_name_vi": "Äá»©c",
        "label": 1,
    },
}

LAYER_MAP = {
    "L1_Dá»…_chÃ­nh_diá»‡n_rÃµ_máº·t": {
        "out_folder": "L1_easy_front_clear",
        "layer_code": "L1",
        "layer_weight": 1.0,
    },
    "L2_BÃ¬nh_thÆ°á»ng_hÆ¡i_nghiÃªng": {
        "out_folder": "L2_normal_slight_angle",
        "layer_code": "L2",
        "layer_weight": 1.2,
    },
    "L3_Biáº¿n_thá»ƒ_gÃ³c_máº·t": {
...
def infer_group_id(filename: str):
    """
    Chá»‘ng leak data:
    áº¢nh augment thÆ°á»ng cÃ³ tÃªn SYN_L4_<source_stem>_aug0001.jpg
    hoáº·c FLIP_..._<source_stem>.jpg.
    Ta cá»‘ gáº¯ng gom source vÃ  augment vÃ o cÃ¹ng group.
    """
    ...
    if stem.startswith("FLIP_"):
        parts = stem.split("_")
        # bá» prefix FLIP_xxx_TO_xxx náº¿u cÃ³
        # giá»¯ pháº§n cuá»‘i lÃ m group tÆ°Æ¡ng Ä‘á»‘i
        if len(parts) > 6:
            stem = "_".join(parts[5:])
...
def assign_splits_groupwise(rows, seed=42, train_ratio=0.75, val_ratio=0.15):
    """
    Chia train/val/test theo group Ä‘á»ƒ trÃ¡nh leak:
    source image vÃ  áº£nh augment tá»« nÃ³ vÃ o cÃ¹ng split.
    """
    random.seed(seed)
